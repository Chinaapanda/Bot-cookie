...
| คีย์ | การกระทำ |
|------|----------|
| `W` / `J` / `Space` | กระโดด (กดค้าง = ค้างจริง) |
| `K` / `S` | สไลด์/มุด (กดค้าง = มุดต่อเนื่องตามที่กด) |
| `Q` / `Esc` หรือ `Ctrl+C` | จบและบันทึก → `patterns/EP1.json` |

> กระโดดสองชั้น (double jump) = กด `W` สองที — ระบบบันทึกตามจริง
> การอัด **บันทึกระยะเวลากดค้างจริง** แล้วตอนเล่นซ้ำจะกดค้างตามนั้นเป๊ะ (ใช้ touch down/up)
